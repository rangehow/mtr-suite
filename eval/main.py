...
if __name__=='__main__':
 ...
 boolean_mask = I[:,0] == np.array(gold_idx)
 equal_indices = np.where(boolean_mask == True)[0]
 unequal_indices = np.where(boolean_mask == False)[0]

 results_to_save = {
 'equal_indices': equal_indices.tolist(),
 'unequal_indices': unequal_indices.tolist()
 }

 # --- 5. JSON ---
 with open('all_indices.json', 'w') as f:
 json.dump(results_to_save, f, indent=4) # indent=4 
 ...
 recall_at_3 = recall(I.tolist(), gold_idx, 3) # Recall at 3
 recall_at_1 = recall(I.tolist(), gold_idx, 1) # Recall at 1
 ...
 metrics_results['recall@3'] = recall_at_3 # Added recall@3
 metrics_results['recall@1'] = recall_at_1 # Added recall@1
