...
if __name__=='__main__':
    ...
    score_names = ['answerable_score','tag_score','faithful_score','quality_score']
    ...
    for judge_model_name in model_names:
        for tested_model_name in model_names:
            for score_name in score_names:
                dataset_dir = os.path.join(args.result_dataset_path, f'{judge_model_name}-{tested_model_name}')
                try:
                    ...
                    if not isinstance(scores, list):
                        ...
                        # Filter out potential non-numeric types and None values
                        numeric_scores = [s for s in scores if isinstance(s, (int, float)) and s is not None]
                        if not numeric_scores:
                             logger.warning(f"No valid numeric scores found in '{score_name}' list in {dataset_dir}.")
                             average_score = np.nan
                             num_scores = 0
                        else:
                            average_score = np.mean(numeric_scores)
                            num_scores = len(numeric_scores)
                            if len(numeric_scores) != len([s for s in scores if s is not None]):
                                logger.warning(f"Non-numeric values (excluding None) found and ignored in '{score_name}' list in {dataset_dir}.")
    ...
    for score_name in results_df['score_name'].unique():
        ...
        # 2. Bar Chart: Average Score Received by each Tested Model
        try:
            avg_score_received = score_df.groupby('tested_model')['average_score'].mean().reindex(model_names).sort_values(ascending=False)
            plt.figure(figsize=(12, 7)) # Keep a slightly larger figure for readability
            ax = avg_score_received.plot(kind='bar', color=sns.color_palette("viridis", len(model_names)))
            ...
            logger.error(f"Failed to generate average score received plot for {score_name}: {e}", exc_info=True)

        # 3. Bar Chart: Average Score Given by each Judge Model
        try:
            avg_score_given = score_df.groupby('judge_model')['average_score'].mean().reindex(model_names).sort_values(ascending=False)
            plt.figure(figsize=(12, 7)) # Keep a slightly larger figure for readability
            ax_given = avg_score_given.plot(kind='bar', color=sns.color_palette("magma", len(model_names)))
            plt.title(f'Average {score_name} Given per Judge Model (Averaged across all Tested Models)')
            ...
            plt.close()
             ...
             logger.error(f"Failed to generate average score given plot for {score_name}: {e}", exc_info=True)
