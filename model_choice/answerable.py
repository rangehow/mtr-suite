...
def generate(llm,args,dataset,sampling_params,tokenizer):
...
def extract_from_outputs(args,outputs,tokenizer):
 ...
 # prompt_token_ids will be deprecated in future versions
...
def extract_numeric_score_from_tag(output_texts,processed_dataset):
 ...
 contexts = record.get('ctxs') # Use.get() for safer dict access
 ground_truth = record.get('ground_truth_ctx')

 # Validate data presence and type
 if contexts is None:
 logger.warning(f"Record {i}: Missing 'ctx' key. Cannot calculate score.")
 elif ground_truth is None:
 logger.warning(f"Record {i}: Missing 'ground_truth_ctx' key. Cannot calculate score.")
 elif not isinstance(contexts, list):
 logger.warning(f"Record {i}: 'ctx' is not a list (type: {type(contexts)}). Cannot calculate score.")
 elif doc_index < 0:
 logger.warning(f"Record {i}: Extracted ID '{extracted_id}' resulted in invalid negative index {doc_index}. Setting score to None.")
 elif doc_index >= len(contexts):
 # Potential IndexError if index is out of bounds
 logger.warning(f"Record {i}: Extracted index {doc_index} is out of bounds for 'ctx' list (size: {len(contexts)}). Extracted ID: '{extracted_id}'. Setting score to None.")
 else:
 # If all checks pass, perform the comparison
 selected_ctx = contexts[doc_index]
 score = (selected_ctx == ground_truth) # Calculate boolean score
 score = 1 if score else 0
 except ValueError:
 logger.warning(f"Record {i}: Could not convert extracted ID '{extracted_id}' to integer. Setting score to None.")
 # Score remains None
 except IndexError:
 # Catch potential index errors accessing processed_dataset[i] itself, though less likely
 logger.error(f"Record {i}: IndexError accessing processed dataset element. This shouldn't happen if length check passed.", exc_info=True)
 # Score remains None
 except KeyError as e:
 # Less likely with.get(), but catches issues if record structure is unexpected
 logger.error(f"Record {i}: KeyError accessing expected data: {e}. Setting score to None.", exc_info=True)
 # Score remains None
 except Exception as e:
 # Catch any other unexpected error during score calculation for this record
 logger.error(f"Record {i}: Unexpected error calculating score for extracted ID '{extracted_id}': {e}", exc_info=True)
 # Score remains None
 finally:
 # Append the calculated score (which could be True, False, or None)
 extracted_scores.append(score)
...
def preprocess(llm,args,sampling_params,tokenizer,dataset,map_func,score_name=None,record_dataset=None):
 ...
 outputs = generate(llm, args, dataset, sampling_params,tokenizer)
...
def judge_answerable(main_dataset, llm, sampling_params, tokenizer, args, score_name):
 """
 Judges the answerability of a query based on the provided context.
 """
 output_texts, _ = preprocess(llm, args, sampling_params, tokenizer, main_dataset, make_judge, score_name, main_dataset)
 extracted_scores = extract_numeric_score_from_range_1_5(output_texts)
 updated_dataset = postprocess(extracted_scores=extracted_scores, score_name=score_name, record_dataset=main_dataset, output_texts=output_texts)
 return updated_dataset


def find_document(main_dataset, llm, sampling_params, tokenizer, args, score_name):
 """
 Identifies the correct document index from a list of contexts.
 """
 # preprocess LLM, map 
 output_texts, processed_for_llm_dataset = preprocess(llm, args, sampling_params, tokenizer, main_dataset, make_find_document, score_name, main_dataset)
 
 #: preprocess processed_for_llm_dataset
 extracted_scores = extract_numeric_score_from_tag(output_texts, processed_for_llm_dataset)
...
def judge_explicit_reference(main_dataset, llm, sampling_params, tokenizer, args, score_name):
 """
 Judges if the response makes an explicit reference.
 """
 output_texts, _ = preprocess(llm, args, sampling_params, tokenizer, main_dataset, make_explicit_reference, score_name, main_dataset)
 extracted_scores = extract_numeric_score_from_range_0_1(output_texts)
 updated_dataset = postprocess(extracted_scores=extracted_scores, score_name=score_name, record_dataset=main_dataset, output_texts=output_texts)
 return updated_dataset


def judge_atomic(main_dataset, llm, sampling_params, tokenizer, args, score_name):
 """
 Judges if the response is atomic.
 """
 output_texts, _ = preprocess(llm, args, sampling_params, tokenizer, main_dataset, make_atomic, score_name, main_dataset)
 extracted_scores = extract_numeric_score_from_range_0_1(output_texts)
 updated_dataset = postprocess(extracted_scores=extracted_scores, score_name=score_name, record_dataset=main_dataset, output_texts=output_texts)
 return updated_dataset


def judge_response_faithful(main_dataset, llm, sampling_params, tokenizer, args, score_name):
 """
 Judges the faithfulness of the response.
 """
 output_texts, _ = preprocess(llm, args, sampling_params, tokenizer, main_dataset, make_response_faithful, score_name, main_dataset)
 extracted_scores = extract_numeric_score_from_range_1_5(output_texts)
 updated_dataset = postprocess(extracted_scores=extracted_scores, score_name=score_name, record_dataset=main_dataset, output_texts=output_texts)
 return updated_dataset


def judge_response_quality(main_dataset, llm, sampling_params, tokenizer, args, score_name):
 """
 Judges the quality of the response.
 """
 output_texts, _ = preprocess(llm, args, sampling_params, tokenizer, main_dataset, make_response_quality, score_name, main_dataset)
 extracted_scores = extract_numeric_score_from_range_1_5(output_texts)
 updated_dataset = postprocess(extracted_scores=extracted_scores, score_name=score_name, record_dataset=main_dataset, output_texts=output_texts)
 return updated_dataset